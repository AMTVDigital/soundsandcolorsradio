 ...
 * 
 * ==========================================================================
 * Create a button to play any resource: radio, post, podcast or aything with audio in it
 * ==========================================================================
 * 
 * 
 */
...
if(!function_exists( 'qtmplayer_play_button' )) {
	function qtmplayer_play_button( $atts = array() ){
		
		
		extract( shortcode_atts( array(
			'button_text' => esc_html__('Play', 'qtmplayer' ),
			'file'		=> false,
		...
		if( false == $file && 'radiochannel' == get_post_type( $id )){
			...
			if( $file ){
				...
				<div class="qtmplayer-trackitem">
					<a <?php qtmplayer_create_track_data( $track_data ); ?> class="qtmplayer-play qtmplayer-link-sec qtmplayer-play-btn qtmplayer-play-v2 <?php echo esc_attr( $item_classes ); ?>">
						<i class="material-icons">play_arrow</i><?php echo esc_html( $button_text ); ?>
					</a>
				</div>
		...
		if( false == $file){
			...
			$_podcast_resourceurl = get_post_meta( intval($id), '_podcast_resourceurl' ,true );
